...
def check_credentials(username, password, root, bg_images):
    ...
    try:
        ...
        if user_row:
            ...
            try:
                user_path = CURRENT_USER_DATA.get('profile_image_path')
                icon_size = 40 # ขนาดไอคอน
                
                if user_path and user_path != '(No file selected)':
                    img = Image.open(user_path)
                    img = img.resize((icon_size, icon_size), Image.LANCZOS)
                    PHOTO_REFERENCES['user_profile_icon'] = ImageTk.PhotoImage(img)
                else:
                    # ถ้าผู้ใช้ไม่มีรูป ให้แน่ใจว่าได้ลบรูปของคนเก่า (ถ้ามี)
                    if 'user_profile_icon' in PHOTO_REFERENCES:
                        del PHOTO_REFERENCES['user_profile_icon']
                        
            except Exception as e:
                print(f"Failed to load user icon during login: {e}")
                if 'user_profile_icon' in PHOTO_REFERENCES:
                    del PHOTO_REFERENCES['user_profile_icon']
...
def create_top_icons(root, bg_images, current_page_func):
    ...
    tk.Button(
        root, image=bg_images['icon_profile'], command=partial(create_profile_page, root, bg_images, current_page_func), 
        bd=0, relief="flat"
...
def save_user_profile(user_id, username, fname, lname, phone, email, address, new_image_path, save_success_func):
    ...
    try:
        ...
        # ✅ [ใหม่] อัปเดตไอคอนที่แคชไว้ทันที (เหมือนในขั้นตอนที่ 1)
        try:
            user_path = CURRENT_USER_DATA.get('profile_image_path')
            icon_size = 40 # ขนาดไอคอน
            ...
            print(f"Failed to reload user icon after save: {e}")
...
def create_pet_details_page(root, bg_images, pet_id, prev_page_func):
    ...
    img_container.place(x=90, y=200, width=230, height=230)
