...
PIC_PROJECT_DIR = os.path.join(BASE_DIR, "PicProjectPet")
...
CATEGORY_ICONS_PATH = "D:\\python\\Project py1\\ProjectPet\\PicProjectPet\\assets\\category_icons"
# --- Image Paths ---
IMAGE_PATHS = {
...
def add_pickup_time_to_orders_table():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        # ใช้ ALTER TABLE เพื่อ "เพิ่ม" คอลัมน์ใหม่
...
def delete_category_from_db(category_id):
    ...
    try:
        ...
        # (เช็คว่ามีสัตว์เลี้ยงตัวไหนยังใช้ type ชื่อ Category นี้อยู่หรือไม่)
        db_cursor.execute(
        ...
        if pet_count > 0:
            # (ถ้ามีสัตว์เลี้ยงใช้อยู่ ห้ามลบ)
            ...
            # (ถ้าไม่มีสัตว์เลี้ยงใช้ ลบได้เลย)
...
def get_next_7_days_pickups():
    ...
    try:
        ...
        # (ดึงข้อมูลWHERE pickup_date อยู่ระหว่างวันนี้ และ วันนี้ +7 วัน)
        db_cursor.execute(
            """
            SELECT 
                o.pickup_date, 
...
def get_sales_report_by_date(start_date, end_date):
    ...
    ตามช่วงวันที่ที่กำหนด (start_date, end_date)
    """
    db_conn = None
    try:
        db_conn = get_db_connection()
        ...
        end_date_str = end_date.strftime('%Y-%m-%d 23:59:59') #รวมเวลาถึงสิ้นวัน
        ...
        return [] # คืนค่าlistว่างเปล่าถ้าพัง
    finally:
        if db_conn:
            db_conn.close()
...
def check_user_credentials(username, password):
    try:
        ...
        user_row_tuple = cursor.fetchone() # ตั้งชื่อใหม่ว่า tuple

        if user_row_tuple:
            # 2.แปลง Tuple เป็น Dict 
            
            # ดึงชื่อคอลัมน์ทั้งหมดจาก cursor
            columns = [col[0] for col in cursor.description]
            
            # จับคู่ชื่อคอลัมน์กับข้อมูลเข้าด้วยกัน
...
def _draw_pdf_table_header(pdf, width, height, FONT_NAME):
    ...
    # เริ่มวาดหัวตารางบนสุดของหน้าใหม่
...
def generate_receipt_pdf(order_id):
    ...
    # 3. เริ่มวาดPDF (Layout ใหม่ทั้งหมด)
    try:
        ...
        # วาด "Customer Info Block" (วาดแค่หน้าแรก)
        current_y = height - 7.5*cm
        pdf.setFont(FONT_NAME, 11)
        ...
        for item in items_in_order:
            ...
            if current_y < bottom_margin:
                pdf.showPage() # ขึ้นหน้าใหม่
                # วาดหัวตาราง(ซ้ำ)
                current_y = _draw_pdf_table_header(pdf, width, height, FONT_NAME)
                current_y -= 0.7*cm 
            ...
            current_y -= 0.3*cm 
            pdf.line(margin_left, current_y, margin_right, current_y) # วาดเส้นคั่นระหว่างไอเทม
            current_y -= 0.4*cm 
            
            item_number += 1
        
        # วาดยอดรวม(หลังจาก Loop จบ)
        if current_y < bottom_margin:
            pdf.showPage()
        ...
        current_y -= 0.3*cm
        pdf.line(margin_left, current_y, margin_right, current_y) # วาดเส้นคั่นใต้Total
...
def get_single_order_details(order_id):
    ...
    try:
        ...
        # ดึงข้อมูล Order หลัก (Join Orders กับ Users)
        db_cursor.execute(
            """
            SELECT o.*, u.username 
            FROM Orders o 
        ...
        # แปลงรายการสัตว์เลี้ยงเป็น list of dicts
...
def get_pets_by_type(pet_type):
    ...
    try:
        ...
        # ตรวจสอบว่า pet_type เป็น None หรือ "All"
        if pet_type is None or pet_type == "All": 
            # ถ้าเป็น "All" หรือ None ให้ดึงทั้งหมด
...
def add_pedigree_path_to_pets_table():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        # ใช้ ALTER TABLE เพื่อเพิ่มคอลัมน์ใหม่
    ...
    try:
        ...
        if order_count > 0:
            ...
            messagebox.showinfo(
                ...
                "กรุณากด 'EDIT' และเปลี่ยนสถานะ (Status) เป็น 'Sold' แทนค่ะ"
            )
            return False # คืนค่าว่า "ล้มเหลว"
            
        else:   # ถ้าไม่เคยถูกสั่งซื้อ ลบได้เลย
    ...
    try:
        ...
        db_cursor.execute(
            ...
            (current_user_id, order_number, final_total, payment_method, 
             ...
             pickup_date, pickup_time) # เพิ่ม pickup_date เข้าไปใน Values
        )
        ...
        db_conn.commit() # ยืนยันทุกอย่างพร้อมกัน
...
def revert_pet_status_to_available(order_id):
    ...
    try:
        ...
        if not pet_id_rows:
            return True # ไม่มีรายการสัตว์เลี้ยงในออเดอร์นี้ถือว่าสำเร็จ
...
def add_pickup_date_to_orders_table():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        # ใช้ ALTER TABLE เพื่อเพิ่มคอลัมน์ใหม่ เข้าไปในตารางที่มีอยู่
...
def load_ctk_image(path, size, fallback_default=False):
    try:
        ...
        image = Image.open(img_path) # 1. เปิดรูปจริง
        ...
        try:
            if fallback_default:
                ...
                # 2. สร้างกรอบรูป (สำรอง)
                ctk_image_obj = ctk.CTkImage(light_image=image, dark_image=image, size=size)

              
                # 3. ผูกรูปจริง (image) ไว้กับ กรอบรูป (ctk_image_obj)
                ctk_image_obj._pil_ref = image 
...
def create_main_app_shell():
    ...
    # 4. หน่วงเวลาการอัปเดตรูปจริงเพื่อให้ปุ่มถูกวาดให้เสร็จก่อน
    ...
    history_icon = load_ctk_image(IMAGE_PATHS['icon_history'], size=(30, 30)) # ปรับsize
...
def create_marketplace_page(parent_frame, selected_category="All", **kwargs):
    ...
    global CATEGORY_ICONS_PATH, IMAGE_PATHS, get_all_pets 
    ...
    # สร้างปุ่ม "All"
    all_icon_path = os.path.join(CATEGORY_ICONS_PATH, "all_icon.png") 
    all_icon = None
...
def create_cart_page(parent_frame, **kwargs):
    ...
    if not pets_in_cart:
        ...
        #  --- (3) "ดัน" footer ลงไปล่างสุด --- 
        spacer_frame.pack(fill="both", expand=True)
    else:
        # ส่วนนี้จะทำงานเฉพาะเมื่อตะกร้ามีของ 
        ...
        header_frame = ctk.CTkFrame(table_container, fg_color="#BCE3FF", height=70)
    ...
    ).pack(side="top", anchor="e", pady=(0, 5)) 
...
def create_purchase_history_page(parent_frame, **kwargs):
    ...
    if not orders_list:
        ...
        for order in orders_list:
            ...
            # --- สร้างกรอบหัวออเดอร์ (สำหรับ Text + ปุ่ม PDF)  ---
            ...
            # วาดหัวตารางของสัตว์เลี้ยง
            table_frame = ctk.CTkFrame(order_container, fg_color="transparent")
            table_frame.pack(fill="x", padx=10, pady=5)
            
            headers = ["Pet", "Breed", "Status", "Pick up date", "Pedigree (PNG)"] 
            COLUMN_WIDTHS = [120, 180, 100, 120, 150] 
            ...
            # --- (4) ดึงสัตว์เลี้ยงที่อยู่ในออเดอร์นี้ ---
            ...
            for pet in pets_in_this_order:
                ...
                pedigree_button = ctk.CTkButton(
                    pet_row_frame, text="View Pedigree (PNG)", font=FONT_NORMAL, width=130, 
...
def create_payment_page(parent_frame, final_total=0, **kwargs):
    ...
    right_scroll_area.pack(fill="both", expand=True, padx=0, pady=5) 
    # สรุปยอด (ไปใส่ใน right_scroll_area)
    ...
    # --- 1.  Customer Information & Date Picker ---
    ...
    fname_entry.insert(0, user_fname)
    ...
    lname_entry.insert(0, user_lname)
    ...
    # Dropdown "ชั่วโมง"
    hour_var = ctk.StringVar(value=PICKUP_HOURS[0])
    ctk.CTkComboBox(
    ...
    # Dropdown "นาที"
    minute_var = ctk.StringVar(value=PICKUP_MINUTES[0]) 
    ctk.CTkComboBox(
...
def create_admin_add_pet_page(parent_frame, pet_id=None, prev_type="DOG", **kwargs):
    ...
    # (Dictionary ชั่วคราว สำหรับเก็บ Path รูป รู2 ป)
    uploaded_paths = {"pet_image": None, "pedigree_image": None}

    if is_edit_mode:
        # (โหมดแก้ไข ดึงข้อมูลเก่ามา)
        pet_data = get_pet_data_by_id(pet_id)
        if not pet_data:
    ...
    def handle_upload_pet_image():
        ...
        try:
            shutil.copy(source_path, destination_path)
            uploaded_paths["pet_image"] = destination_path 
    ...
    def handle_upload_pedigree_image():
        ...
        try:
            shutil.copy(source_path, destination_path)
            uploaded_paths["pedigree_image"] = destination_path
    ...
    ctk.CTkLabel(form_frame, text=page_title, font=FONT_TITLE).pack(pady=10) 
    ...
    if is_edit_mode: type_combo.set(pet_data.get('type', '')) 
    ...
    if is_edit_mode: breed_entry.insert(0, pet_data.get('breed', '')) 
    ...
    if is_edit_mode: age_entry.insert(0, pet_data.get('age', '')) 
    
    ctk.CTkLabel(form_grid, text="Gender", font=FONT_BOLD).grid(row=2, column=1, sticky="w", pady=(10,0), padx=10)
    gender_var = ctk.StringVar(value=pet_data.get('gender', 'Male')) 
    ...
    if is_edit_mode: color_entry.insert(0, pet_data.get('color', '')) 
    ...
    if is_edit_mode: price_entry.insert(0, pet_data.get('price', '')) 
    ...
    if is_edit_mode: other_textbox.insert("1.0", pet_data.get('other', '')) 
    ...
    if is_edit_mode: pet_image_status.configure(text=os.path.basename(uploaded_paths["pet_image"] or ""), text_color="green") 
    ...
    if is_edit_mode: pedigree_image_status.configure(text=os.path.basename(uploaded_paths["pedigree_image"] or ""), text_color="green") 
    
    # (Row 7: Status)
    ctk.CTkLabel(form_grid, text="Status", font=FONT_BOLD).grid(row=10, column=0, sticky="w", pady=(10,0))
    status_var = ctk.StringVar(value=pet_data.get('status', 'Available')) 
    ...
    # (Row 8: Save Button)
    ctk.CTkButton(form_grid, text=button_text, font=FONT_BOLD, height=40,
...
def create_admin_pet_list_page(parent_frame, initial_type="DOG", **kwargs):
    ...
    def display_pets():
        ...
        cols = 5
        for i in range(cols):
            scroll_area.grid_columnconfigure(i, weight=1, uniform="col")  
...
def create_admin_category_page(parent_frame, **kwargs):
    ...
    def handle_upload_category_icon():
        ...
        try:
            # (เช็คว่า Path ต้นทาง กับ Path ปลายทางเหมือนกันหรือไม่)
            if os.path.samefile(source_path, destination_path):
                # (ถ้าใช่ไม่ต้องคัดลอก"แค่จำPathไว้ก็พอ)
            ...
            # (แปลว่าไฟล์ปลายทางยังไม่มี ก็ไปคัดลอกตามปกติ)
            pass
        except Exception as e:
            print(f"Error checking samefile: {e}")
            pass # (ไปคัดลอกตามปกติ)
        # -----------------------------------------------

        # ถ้าไม่เหมือนก็คัดลอก(Copy) ตามปกติ
        try:
            shutil.copy(source_path, destination_path)
            new_icon_path_var.set(destination_path) 
            icon_status_label.configure(text=new_filename, text_color="green")
    ...
    # --- ฟังก์ชันกด Add(เพิ่มประเภทใหม่) ---
    def handle_add_category():
        ...
        if success:
            ...
            handle_show_admin_page(create_admin_category_page) 
        else:
            messagebox.showerror("Error", "Failed to add category (it might already exist).")

    # --- ฟังก์ชันกด Delete(ลบประเภท) ---
...
def get_all_orders_admin():
    ...
    try:
        ...
        # (1. เอา Pending ขึ้นก่อน, 2. เรียงตามวันที่ใหม่สุด)
        db_cursor.execute(
            """
            SELECT * FROM Orders 
            ORDER BY 
...
def create_admin_order_list_page(parent_frame, **kwargs):
    ...
    for order in orders_list:
        ...
        status_color = "#FFA000" if status == 'Pending' else "#4CAF50" 
...
def create_admin_order_details_page(parent_frame, order_id, **kwargs):
    ...
    # --- สร้างฟังก์ชัน Handler สำหรับปุ่ม ---
    ...
    def handle_cancel_order():
        ...
        # ตรวจสอบว่า order_id ถูกนิยามไว้ใน scope ของ create_admin_order_details_page แล้ว
...
def create_admin_dashboard_page(parent_frame, **kwargs):
    ...
    # --- ดึงสถิติจาก DB (เหมือนเดิม) ---
    stats = get_dashboard_stats()

    # (1. สร้างกรอบเลื่อนหลักห้ามgrid() ที่ตัวนี้)
    main_scroll_frame = ctk.CTkScrollableFrame(parent_frame, fg_color="#f0f2f5")
    main_scroll_frame.pack(fill="both", expand=True)

    # --- 2. สร้างกรอบจัดตาราง (Grid Container) ข้างในกรอบเลื่อน  ---
    ...
    # --- 3. ย้าย .grid_columnconfigure() ไปไว้ที่กรอบใหม่ ---
    grid_container.grid_columnconfigure((0, 1, 2), weight=1) 

    # (4. วาง Title ในกรอบใหม่(grid_container))
    ...
    # --- วาดการ์ด 6 ใบ (Row 1 และ 2) ---
    
    # (5. วางการ์ดใน "กรอบใหม่" (grid_container))
    
    # (แถวที่ 1)
    create_stat_card(
        parent=grid_container, row=1, column=0, 
    ...
    create_stat_card(
        parent=grid_container, row=1, column=1, 
    ...
    create_stat_card(
        parent=grid_container, row=1, column=2, 
    ...
    # (แถวที่ 2)
    create_stat_card(
        parent=grid_container, row=2, column=0, 
    ...
    create_stat_card(
        parent=grid_container, row=2, column=1, 
    ...
    create_stat_card(
        parent=grid_container, row=2, column=2, 
    ...
    # ---  สรุปปฏิทินนัดรับ 7 วัน (Row 3) ---
    
    # (6. วางกรอบนี้ในกรอบใหม่ (grid_container))
    pickup_frame = ctk.CTkFrame(grid_container, fg_color="#FFFFFF", corner_radius=10) 
    ...
    # ปุ่มดูทั้งหมดที่จะลิงก์ไปหน้าปฏิทินเต็ม
    ctk.CTkButton(
        pickup_header_frame, text="View Full Calendar...", font=FONT_NORMAL,
        fg_color="transparent", text_color="#007bff",
        command=lambda: handle_show_admin_page(create_admin_calendar_page) 
    ).pack(side="right")
...
def create_admin_calendar_page(parent_frame, **kwargs):
    ...
    # (กล่องข้อความเอารายละเอียดลงในนี้)
    details_textbox = ctk.CTkTextbox(details_frame, font=FONT_NORMAL, state="disabled") 
    ...
    def update_calendar_events():
        ...
        # (สร้างformat ที่ SQL STRFTIME ต้องการ "YYYY-MM")
        month_str = f"{current_year:04d}-{current_month:02d}"
        # --------------------------------------------------
        
        # เรียกเครื่องยนต์ DB (เวอร์ชัน STRFTIME)
        monthly_pickups = get_pickups_for_month(month_str) 
        
        # วนลูปคิวแล้ว"มาร์ค"(calevent_create)
    ...
    def on_date_select(event=None):
        ...
        # (ค้นหาคิว (จาก 'monthly_pickups') ที่ตรงกับวันที่เลือก)
        found_pickups = []
        for pickup in monthly_pickups:
            pickup_date = datetime.datetime.strptime(pickup['pickup_date'], '%Y-%m-%d').date()
    ...
    # (เมื่อเลือกวันที่ให้เรียก on_date_select)
    cal.bind("<<CalendarSelected>>", on_date_select)  
    # (เมื่อเปลี่ยนเดือน(คลิกลูกศร)ให้เรียก update_calendar_events)
    cal.bind("<<CalendarMonthChanged>>", lambda e: update_calendar_events())
    # (โหลดคิวของเดือนปัจจุบัน(ครั้งแรก)ตอนที่หน้าเปิด)
...
def create_admin_sales_report_page(parent_frame, **kwargs):
    ...
    report_scroll_frame.grid_columnconfigure(0, weight=3) 
    report_scroll_frame.grid_columnconfigure(1, weight=1) 
    report_scroll_frame.grid_columnconfigure(2, weight=1) 
    
    # (วาดหัวตารางของรายงาน)
    ...
    # --- ฟังก์ชัน "Generate" (เมื่อกดปุ่ม) ---
    def generate_report():
        # 1. ล้างแถวเก่าทิ้ง (ยกเว้นแถว "หัวตาราง" (row=0))
        for widget in report_scroll_frame.winfo_children():
            if int(widget.grid_info()['row']) > 0: # (ถ้าแถว > 0)
                widget.destroy()

        # 2. ดึงวันที่จากปฏิทิน
...
# ======================================================================
def main():
    global ROOT, MAIN_CONTAINER, CURRENT_USER, CART  
    # ตรวจสอบ Path หลัก
