...
# ฟังก์ชัน handle_pet_click
def handle_pet_click(pet_id):
    # ฟังก์ชันนี้จะถูกเรียกเมื่อคลิกที่การ์ดสัตว์
    print(f"เปิดหน้ารายละเอียดสัตว์ที่มี ID: {pet_id}")
    # เพิ่มการเปิดหน้ารายละเอียดสัตว์ที่ต้องการ
    # เช่น: create_pet_details_page(root, bg_images, pet_id)
...
def check_credentials(username, password, root, bg_images):
    ...
    try:
        ...
        if user_row:
            ...
            create_menu_and_animal_list_page(root, bg_images, user_role='user') 
        else:
...
# ======================================================================
# หน้า Animal List +Main Menu (ส่วนที่แก้ไข)
# ======================================================================
...
# วางโค้ดนี้แทนที่:
def create_menu_and_animal_list_page(root, bg_images, user_role='user'):
    clear_page(root)
    root.title("PET PARADISE - Menu and Animal List")

    try:
        if 'menu2' not in bg_images:
            menu_img = Image.open("D:\\PicProjectPet\\menu2.png")
            bg_images['menu2'] = ctk.CTkImage(
                light_image=menu_img,
                size=(WINDOW_WIDTH, WINDOW_HEIGHT)
            )    
    except Exception as e:
        messagebox.showerror("Error", f"โหลดภาพ menu2 ผิดพลาด: {e}")
        return

    bg_label = ctk.CTkLabel(root, image=bg_images['menu2'], text="")
    ...
    # สร้างปุ่มหมวดหมู่สัตว์
    for i, (label, pet_type) in enumerate(PET_MENU):
        btn = ctk.CTkButton(
            root,
            text=label,
            font=("Arial Rounded Bold", 22),
            width=300,
            height=60,
            fg_color="#5fa7d1",
            hover_color="#4a86b9",
            text_color="white",
            corner_radius=20,
            command=lambda p=pet_type: display_pets(root, bg_images, p)  # เมื่อคลิกจะเปลี่ยนประเภทสัตว์
        )
        btn.place(relx=0.5, y=start_y + (i * spacing_y), anchor="center")

    # พื้นที่แสดงรายการสัตว์ (กรณีมีการแสดงผลแบบตารางหรือการ์ด)
    
    display_frame_container = tk.Frame(root, bg='white')
    display_frame_container.place(x=15, y=102, width=930, height=365) 
    

    # ใช้ Canvas และ Scrollbar
    ...
    # ฟังก์ชันแสดงรายการสัตว์
    def display_pets(root, bg_images, pet_type):
        for widget in display_frame.winfo_children():
            widget.destroy()

        pets = get_pets_by_type(pet_type)  # ฟังก์ชันดึงข้อมูลสัตว์
        
        if not pets:
            tk.Label(display_frame, text=f"No {pet_type} items currently available.", font=("Josefin Sans Bold", 14), bg="white").pack(pady=50)
            return

        COLUMNS = 4  # จำนวนคอลัมน์ในการจัดวางสัตว์

        for i, pet in enumerate(pets):
            pet_id, pet_type_db, breed, gender, age, color, price, image_key, status, other = pet
            row = i // COLUMNS
            col = i % COLUMNS
            # โค้ดที่แสดงการ์ด (card) และการเชื่อมโยงการคลิก
            card = tk.Frame(display_frame, bg="white", bd=0, relief="flat", highlightbackground="#d9e8ff", highlightthickness=2)
            card.grid(row=row, column=col, padx=15, pady=15, sticky="n")
            card.bind("<Button-1>", lambda e, pid=pet_id: handle_pet_click(pid))# เมื่อคลิกที่การ์ดจะเรียก handle_pet_click

            img = load_pet_image(image_key, size=(150,150))
            img_label = ctk.CTkLabel(card, image=img, text="", fg_color="white")
            img_label.image = img
            img_label.pack(pady=(10,5))
            
            # (อย่าลืม import: from customtkinter import CTkFont)

    # --- 1. สร้างอ็อบเจกต์ฟอนต์ที่ต้องใช้ก่อน ---
            font_ribbon = CTkFont(family="Josefin Sans", size=10, weight="bold")
            font_breed = CTkFont(family="Josefin Sans", size=11, weight="bold")
            font_age = CTkFont(family="Josefin Sans", size=9)
            font_price = CTkFont(family="Josefin Sans", size=11, weight="bold") # (เหมือน font_breed)

            # --- 2. สร้าง Label ใหม่ทั้งหมด ---
    
            ribbon_text = "AVAILABLE" if status == "Available" else "SOLD"
            ribbon_bg = "#77c0ff" if status == "Available" else "#ff9bb3"
    
            ribbon = ctk.CTkLabel(card, 
                          text=ribbon_text, 
                          fg_color=ribbon_bg,   # <-- แก้จาก bg
                          text_color="white", # <-- แก้จาก fg
                          font=font_ribbon      # <-- แก้เป็น CTkFont
                          # (ลบ padx, pady ออก)
                         )
            ribbon.place(relx=0.5, y=0, anchor="n") # .place() ยังใช้ได้เหมือนเดิม

            lbl_breed = ctk.CTkLabel(card, 
                             text=f"{breed}", 
                             font=font_breed,       # <-- แก้เป็น CTkFont
                             text_color="#3b6fa3",  # <-- แก้จาก fg
                             fg_color="white"       # <-- แก้จาก bg
                            )
            lbl_breed.pack()

            lbl_age = ctk.CTkLabel(card, 
                           text=f"{age} months", 
                           font=font_age,         # <-- แก้เป็น CTkFont
                           text_color="#666",     # <-- แก้จาก fg
                           fg_color="white"       # <-- แก้จาก bg
                          )
            lbl_age.pack(pady=(0,3)) # pady ใน .pack() ยังใช้ได้เหมือนเดิม

            price = float(price)
            price_tag = ctk.CTkLabel(card, 
                             text=f"฿{price:,.0f}", 
                             font=font_price,       # <-- แก้เป็น CTkFont
                             fg_color="#5fa7d1",    # <-- แก้จาก bg
                             text_color="white"     # <-- แก้จาก fg
                             # (ลบ padx, pady ออก)
                            )
    ...
    # เริ่มต้นแสดงรายการจาก "All Pets"
    display_pets(root, bg_images, "ALL")
...
def create_profile_page(root, bg_images, prev_page_func):
    ...
    background_label = ctk.CTkLabel(root, image=bg_images['profile'], text="") # <-- แก้เป็น ctk.CTkLabel และเพิ่ม text=""
    background_label.place(x=0, y=0, relwidth=1, relheight=1)
...
def create_edit_profile_page(root, bg_images, prev_page_func):
    ...
    background_label = ctk.CTkLabel(root, image=bg_images['edit_profile'], text="") # <-- แก้เป็น ctk.CTkLabel และเพิ่ม text=""
    background_label.place(x=0, y=0, relwidth=1, relheight=1)
...
def create_pet_details_page(root, bg_images, pet_id, prev_page_func):
    ...
    background_label = ctk.CTkLabel(root, image=bg_images['pet_detail'], text="") # <-- แก้เป็น ctk.CTkLabel และเพิ่ม text=""    
    background_label.place(x=0, y=0, relwidth=1, relheight=1)
...
# ======================================================================
# หน้า Menu ตอนนี้เอาไปรวมกับanimal list
# ======================================================================
...
def create_admin_pet_list_page(root, bg_images, initial_type="DOG"):
    ...
    background_label = ctk.CTkLabel(root, image=bg_images['dog_list'], text="") # <-- แก้เป็น ctk.CTkLabel และเพิ่ม text=""
    background_label.place(x=0, y=0, relwidth=1, relheight=1)
    # ----------------------------------------------------------------------
    # --- [Admin Actions] ---
    # ----------------------------------------------------------------------
...
def create_admin_pet_form_page(root, bg_images, prev_page_func, pet_id=None):
    ...
    background_label = ctk.CTkLabel(root, image=bg_images[bg_key], text="") # <-- แก้เป็น ctk.CTkLabel และเพิ่ม text=""
    background_label.place(x=0, y=0, relwidth=1, relheight=1)
...
def create_main_window():
    ...
    root = ctk.CTk()
    root.geometry(f"{WINDOW_WIDTH}x{WINDOW_HEIGHT}") # <--- เพิ่มบรรทัดนี้
    root.title("PET PARADISE") # (หรือ title อะไรก็ตามที่คุณตั้ง)
