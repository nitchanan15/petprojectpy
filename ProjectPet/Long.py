...
def get_pickups_for_month(month_str):
    ...
    try:
        ...
        # (เราจะดึงข้อมูลที่ "จำเป็น" ทั้งหมด... 
        # ...เฉพาะออเดอร์ที่ "ยังไม่เสร็จ" (Pending) หรือ "เพิ่งเสร็จ" (Completed))
        db_cursor.execute(
            """
            SELECT 
                o.pickup_date, 
    ...
    try:
        ...
        db_cursor.execute(
            ...
            WHERE 
                ...
                AND o.order_status = 'Pending' -- (เราสนแค่ "คิว" ที่ยังไม่เสร็จ)
            GROUP BY o.order_id
            ORDER BY o.pickup_date, o.pickup_time
            """,
        )
...
def generate_receipt_pdf(order_id):
    ...
    try:
        FONT_PATH = "D:\\PicProjectPet\\assets\\FkDragonDemo.ttf"
        FONT_NAME = "FkDragon" 
...
# --- Pet Functions ---
# (วางทับ def get_pets_by_type ใน database.py)
def get_pets_by_type(pet_type):
    """ดึงสัตว์เลี้ยงตามประเภท หรือดึงทั้งหมดหากเป็น 'All'"""
    db_conn = None
    try:
        db_conn = get_db_connection()
        ...
        if pet_type == "All":
            # ‼️ (FIX) ถ้าเป็น "All"... ดึงทั้งหมด (ไม่ใส่ WHERE clause)
            cursor.execute("SELECT * FROM pets ORDER BY breed")
        else:
            # (ถ้าเป็นประเภทปกติ... ใช้ WHERE)
    ...
    try:
        ...
        db_cursor.execute(
            ...
            (current_user_id, order_number, final_total, payment_method, 
             ...
             pickup_date) # ‼️ 2. เพิ่ม pickup_date เข้าไปใน Values
        )
...
def handle_show_admin_page(PageFunc, **kwargs):
    """(Admin) สลับหน้าในกรอบเนื้อหาของแอดมิน (เวอร์ชัน "clear_frame")"""
    global ADMIN_CONTENT_FRAME

    if not ADMIN_CONTENT_FRAME:
        print("Error: ADMIN_CONTENT_FRAME is not initialized.")
        return

    # --- ‼️ (NEW) "ลบลูก" (Clear Children) ... "ไม่" ลบแม่ (Destroy) ‼️ ---
    clear_frame(ADMIN_CONTENT_FRAME) 
    print("ROOT children =", [child for child in ROOT.winfo_children()])
    print("ADMIN children =", [child for child in ADMIN_CONTENT_FRAME.winfo_children()])

    # (เรียกฟังก์ชันสร้างหน้า... ให้ไป "วาด" ลงใน "กรอบที่ว่าง")
    PageFunc(ADMIN_CONTENT_FRAME, **kwargs)

# (นี่คือเวอร์ชันอัปเกรด... วางทับอันเก่าได้เลย)
def create_admin_app_shell():
    global ADMIN_CONTENT_FRAME, ROOT
    ...
    # NAVIGATION (LEFT)
    # ----------------------------------------------------
    nav_frame = ctk.CTkFrame(ROOT, width=200, fg_color="#2c3e50", corner_radius=0)
    ...
    # Buttons
    ctk.CTkButton(nav_frame, text="Dashboard", font=FONT_BOLD,
        command=lambda: handle_show_admin_page(create_admin_dashboard_page)
    ).pack(fill="x", padx=10, pady=5)

    ctk.CTkButton(nav_frame, text="Sales Report", font=FONT_BOLD,
        command=lambda: handle_show_admin_page(create_admin_sales_report_page)
    ).pack(fill="x", padx=10, pady=5)

    ctk.CTkButton(nav_frame, text="View Orders", font=FONT_BOLD,
        command=lambda: handle_show_admin_page(create_admin_order_list_page)
    ).pack(fill="x", padx=10, pady=5)

    ctk.CTkButton(nav_frame, text="Pickup Calendar", font=FONT_BOLD,
        command=lambda: handle_show_admin_page(create_admin_calendar_page)
    ).pack(fill="x", padx=10, pady=5)

    ctk.CTkButton(nav_frame, text="View All Pets", font=FONT_BOLD,
        command=lambda: handle_show_admin_page(create_admin_pet_list_page)
    ).pack(fill="x", padx=10, pady=5)

    ctk.CTkButton(nav_frame, text="Add New Pet", font=FONT_BOLD,
        command=lambda: handle_show_admin_page(create_admin_add_pet_page)
    ).pack(fill="x", padx=10, pady=5)

    ctk.CTkButton(nav_frame, text="Manage Categories", font=FONT_BOLD,
        command=lambda: handle_show_admin_page(create_admin_category_page)
    ).pack(fill="x", padx=10, pady=5)

    # Logout bottom
    ctk.CTkButton(nav_frame, text="Logout", fg_color="#ff7676", hover_color="#c74c4c",
    ...
    # CONTENT AREA (RIGHT)
    # ----------------------------------------------------
    ADMIN_CONTENT_FRAME = ctk.CTkFrame(ROOT, fg_color="transparent")
...
def create_home_page(parent_frame, **kwargs):
    ...
    bg_label = ctk.CTkLabel(ADMIN_CONTENT_FRAME, image=bg_image, text="")
...
# --- PAGE: create_marketplace_page (เวอร์ชัน "Final" - ดึง Category จาก DB) ---
...
def show_pedigree_popup(parent_frame, pedigree_path, breed_name):
    ...
    if not pedigree_path or not os.path.exists(pedigree_path):
        messagebox.showwarning("No Pedigree", "ขออภัย, ยังไม่มีไฟล์เพ็ดดีกรีสำหรับสัตว์เลี้ยงตัวนี้ครับ")
        return
...
# --- PAGE: create_admin_pet_list_page (เวอร์ชัน "แก้บั๊ก" APP_CONTENT_FRAME) ---
def create_admin_pet_list_page(parent_frame, initial_type="DOG", **kwargs):
    global PET_CATEGORIES, IMAGE_PATHS, FONT_BOLD, FONT_NORMAL # (เพิ่ม Globals)
    ...
    # --- (ดึงประเภทจาก DB) ---
    all_categories = get_all_categories() # ‼️ (NEW) ดึงจาก DB
    categories_list = ["All"] + [cat['name'] for cat in all_categories] # ‼️ (สร้าง List ใหม่)
    
    # --- Nested Function: delete_pet (เหมือนเดิม) ---
    ...
    # (ใน def create_admin_pet_list_page)
    # ... (โค้ด tab_buttons = {} และ scroll_area = ... อยู่ข้างบน) ...

    # --- Nested Function: display_pets (New Layout) ---
    def display_pets():
        clear_frame(scroll_area)
        pet_type = current_type_var[0]
        
        # ‼️ (NEW) "ถ้า" เป็น All ... ให้เรียก get_all_pets() ‼️
        if pet_type == "All":
            pets = get_all_pets(selected_category=None) # (สมมติว่า get_all_pets รับ None เพื่อดึงทั้งหมด)
        else:
        ...
        for i, pet in enumerate(pets):
            ...
            # --- ‼️ (NEW) Pet Card Layout (เหมือนหน้า Marketplace) ‼️ ---
            card = ctk.CTkFrame(scroll_area, width=200, height=260, fg_color="#ffffff", corner_radius=12)
            card.grid(row=row, column=col, padx=10, pady=10, sticky="nsew") 

            # (รูปสัตว์เลี้ยง)
            img = load_ctk_image(pet['image_key'], size=(180, 180), fallback_default=True)
            img_label = ctk.CTkLabel(card, image=img, text="", fg_color="transparent")
            img_label.place(x=10, y=10, width=180, height=180) 
            if img: img_label.image = img
            ...
            # (ข้อมูล Text)
            ctk.CTkLabel(card, text=f"Breed: {pet['breed']}", font=FONT_BOLD, text_color="#003b63").place(x=10, y=200)
            ctk.CTkLabel(card, text=f"Price: ฿{pet['price']:,.0f}", font=FONT_NORMAL, text_color="#005f99").place(x=10, y=225)
            
            # --- ‼️ (NEW) ปุ่ม Edit/Delete (วางไว้ล่างสุด) ‼️ ---
            button_frame = ctk.CTkFrame(card, fg_color="transparent")
            button_frame.place(x=10, y=240) # (วางไว้ใต้ข้อมูล)
            
            # (ปุ่ม Edit)
            ctk.CTkButton(
                button_frame, text="EDIT", font=("Arial", 10), width=60, height=20,
                command=lambda p_id=pet['id']: handle_show_admin_page(create_admin_add_pet_page, pet_id=p_id, prev_type=pet_type)
            ).pack(side="left", padx=2)
            
            # (ปุ่ม Delete)
            ctk.CTkButton(
                button_frame, text="DELETE", font=("Arial", 10), width=60, height=20,
                fg_color="#ff7676", hover_color="#c74c4c",
                command=lambda p_id=pet['id']: delete_pet(p_id, display_pets)
            ).pack(side="left", padx=2)
            
            # (เพิ่มคำสั่งคลิก Card... เพื่อให้เป็นไปตาม Layout เดิมของคุณ)
            card.bind("<Button-1>", lambda event, p_id=pet['id'], cat=pet_type: handle_show_admin_page(create_admin_add_pet_page, pet_id=p_id, prev_type=cat))
    ...
    def handle_tab_click(pet_type):
        ...
        # ‼️ (แก้ไข) ใช้ categories_list เพื่อเทียบสีปุ่ม ‼️
        for pt in categories_list:
             btn = tab_buttons.get(pt)
             if btn:
...
# --- PAGE: create_admin_category_page ---
...
def create_admin_order_details_page(parent_frame, order_id, **kwargs):
    ...
    def handle_view_slip():
        ...
        # (เรา "ใช้ซ้ำ" ฟังก์ชัน popup ที่เราสร้างไว้สำหรับ "เพ็ดดีกรี" ได้เลย!)
...
def create_admin_calendar_page(parent_frame, **kwargs):
    ...
    def update_calendar_events():
        ...
        nonlocal monthly_pickups # (อ้างอิงตัวแปร "ชั่วคราว")
        
        # (ล้างมาร์คเก่าทิ้ง)
        cal.calevent_remove('all') 
        
        # (ดึง "เดือน" ที่ปฏิทินกำลังโชว์)
        current_year, current_month = cal.get_displayed_month()
        
        # (สร้าง "format" ที่ SQL ต้องการ ... "YYYY-MM")
        month_str = f"{current_year:04d}-{current_month:02d}"
        
        # (เรียก "เครื่องยนต์" DB)
        ...
        for pickup in monthly_pickups:
            try:
                ...
                # (เราจะ "มาร์ค" วันนั้นด้วย "แท็ก" ชื่อ "pickup")
                cal.calevent_create(pickup_date, "Pickup Scheduled", "pickup") 
            except Exception as e:
                print(f"Error marking date: {e}")
        
        # (ตั้งค่า "สี" ของ "แท็ก")
        cal.tag_config("pickup", background="#FFA000", foreground="white") # (สีส้ม)
...
def main():
    ...
    #add_pickup_date_to_orders_table()
    #add_pedigree_path_to_pets_table()
    #add_pickup_time_to_orders_table()
    create_categories_table()
    #migrate_old_categories(PET_CATEGORIES, CATEGORY_ICONS_PATH)
    #add_sort_order_to_categories_table()
    #update_initial_category_order(PET_CATEGORIES)
