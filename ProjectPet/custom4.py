...
def create_pet_details_page(root, bg_images, pet_id, prev_page_func):
    ...
    # ---- พื้นหลังใหม่ ----
    try:
        bg_img = ImageTk.PhotoImage(Image.open("D:\\PicProjectPet\\petde.png"))
    except:
        messagebox.showerror("Error", "ไม่พบรูป petde.png")
        prev_page_func()
        return
    
    tk.Label(root, image=bg_img).place(x=0, y=0, relwidth=1, relheight=1)
    root.bg_img = bg_img  # กันรูปหลุดจากหน่วยความจำ

    # ---- รูปสัตว์ใหญ่ซ้าย ----
    pet_img = load_pet_image(pet_data['image_key'], size=(300, 300))

    frame_img = tk.Frame(root, bg="white", bd=5, relief="ridge")
    frame_img.place(x=60, y=160, width=330, height=330)

    if pet_img:
        lbl_img = tk.Label(frame_img, image=pet_img, bg="white")
        lbl_img.image = pet_img
        lbl_img.place(relx=0.5, rely=0.5, anchor="center")
    else:
        tk.Label(frame_img, text="No Image", bg="white").place(relx=0.5, rely=0.5, anchor="center")

    # ---- ข้อมูลสัตว์ด้านขวา ----
    font_label = ("Josefin Sans Bold", 18)
    x = 430
    y = 170
    gap = 50

    tk.Label(root, text=f"Breed: {pet_data['breed']}", font=font_label, bg="white", fg="#333").place(x=x, y=y)
    tk.Label(root, text=f"Age: {pet_data['age']}", font=font_label, bg="white", fg="#333").place(x=x, y=y+gap)
    tk.Label(root, text=f"Gender: {pet_data['gender']}", font=font_label, bg="white", fg="#333").place(x=x, y=y+2*gap)
    tk.Label(root, text=f"Color: {pet_data['color']}", font=font_label, bg="white", fg="#333").place(x=x, y=y+3*gap)
    tk.Label(root, text=f"Price: {pet_data['price']} ฿", font=font_label, bg="white", fg="#d9534f").place(x=x, y=y+4*gap)
    tk.Label(root, text=f"Other: {pet_data['other']}", font=font_label, bg="white", fg="#333").place(x=x, y=y+5*gap)
    ...
    def add_cart():
        if pet_data['status'] == "Sold":
            ...
            messagebox.showinfo("Cart", f"{pet_data['breed']} added to cart ✅")
    
    tk.Button(root, text="Add to Cart", font=("Josefin Sans Bold", 16), 
              command=add_cart, bg="#5fa7d1", fg="white", bd=0).place(
              x=810, y=520, width=150, height=48)

    # ---- About Button ----
    tk.Button(root, text="About", font=("Josefin Sans Bold", 14), 
              command=lambda: messagebox.showinfo("About", "สัตว์น่ารักมากกก!"), 
              bg="#82c4a8", fg="white", bd=0).place(
              x=630, y=520, width=150, height=48)
