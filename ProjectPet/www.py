...
# (นี่คือฟังก์ชันใหม่ที่คุณต้องสร้าง)
def create_main_app_shell(root, bg_images, user_data):
    """
    สร้างกรอบหลัก (Shell) ของแอป: Top Bar (ถาวร) และ Content Frame (สำหรับเปลี่ยนหน้า)
    """
    # 1. ล้างหน้า Login/Home ออกจาก 'root' (นี่จะเป็นการ clear_page(root) ครั้งสุดท้าย)
    clear_page(root) 
    root.title("Pet Paradise")

    # 2. สร้าง "Top Bar" (แถบไอคอนถาวร)
    #    เราใช้ .pack() เพื่อให้มันยึดด้านบนสุดของ 'root' เสมอ
    top_bar_frame = ctk.CTkFrame(root, height=70, fg_color="#FFFFFF", corner_radius=0) # (สี/ความสูง ปรับได้)
    top_bar_frame.pack(side="top", fill="x")

    # 3. สร้าง "Content Frame" (พื้นที่เนื้อหา)
    #    นี่คือกรอบที่จะ "กินพื้นที่" ที่เหลือทั้งหมด และเป็นที่ที่เราจะวาดหน้าต่างๆ
    content_frame = ctk.CTkFrame(root, fg_color="transparent")
    content_frame.pack(side="top", fill="both", expand=True)

    # 4. เรียกใช้ create_top_icons (เวอร์ชันใหม่) 
    #    โดยสั่งให้มันวาดไอคอนลงใน 'top_bar_frame' 
    #    และส่ง 'content_frame' ไปให้มันด้วย
    create_top_icons(top_bar_frame, content_frame, bg_images, user_data, root) 
    
    # 5. โหลดหน้าแรก (Marketplace) ลงใน "Content Frame"
    #    (เราต้องส่ง user_role ไปด้วย)
    create_marketplace_page(content_frame, bg_images, user_data['role'])

# (นี่คือเวอร์ชันแก้ไขของฟังก์ชันที่ 3)
def create_top_icons(top_bar, content_frame, bg_images, user_data, root):
    """
    สร้างไอคอน Profile, Cart, และ Logout ลงใน 'top_bar' (แถบถาวร)
    """
    # --- 1. สร้าง "ทางกลับบ้าน" (prev_page_func) ---
    #    นี่คือฟังก์ชันที่ปุ่ม Back ในหน้า Cart/Profile จะใช้
    #    สังเกตว่ามันเรียก Marketplace ลงใน 'content_frame'
    prev_page_func = partial(create_marketplace_page, 
                             content_frame, 
                             bg_images, 
                             user_data['role'])

    # --- 2. สร้างปุ่ม Logout (ไว้ซ้ายสุด) ---
    # (ปุ่มนี้ "พิเศษ" เพราะมันต้องกลับไปหน้า Home ที่อยู่บน 'root')
    home_page_func = partial(create_home_page, root, bg_images)
    
    ctk.CTkButton(
        top_bar, # 👈 วาดลงใน top_bar
        text="Logout",
        width=80, height=30,
        fg_color="#ff7676", hover_color="#c74c4c",
        command=home_page_func # 👈 กลับไปหน้า Home (ทำลาย Shell นี้)
    ).pack(side="left", padx=20, pady=10) # 👈 ใช้ .pack()

    # --- 3. สร้างปุ่ม Cart (ไว้ขวาสุด) ---
    ctk.CTkButton(
        top_bar, # 👈 วาดลงใน top_bar
        ...
        width=40, height=40,
        # 👈 Command ใหม่: เรียก create_cart_page ลงใน 'content_frame'
        command=partial(create_cart_page, content_frame, bg_images, prev_page_func)
    ).pack(side="right", padx=10, pady=10) # 👈 ใช้ .pack()

    # --- 4. สร้างปุ่ม Profile (ไว้ขวาสุด ถัดจาก Cart) ---
    # (โค้ดโหลดรูปโปรไฟล์ของคุณ... ย้ายมาไว้ตรงนี้)
    profile_icon_image = None
    icon_size = (40, 40)
    user_path = user_data.get('profile_image_path')
    ...
    if not profile_icon_image:
        profile_icon_image = bg_images.get('icon_profile')
    ...
    ctk.CTkButton(
        ...
        fg_color="transparent",
        width=40, height=40,
        # 👈 Command ใหม่: เรียก create_profile_page ลงใน 'content_frame'
        command=partial(create_profile_page, content_frame, bg_images, prev_page_func) 
        # (หมายเหตุ: create_profile_page อาจต้องรับ user_data ด้วย)
    ).pack(side="right", padx=10, pady=10) # 👈 ใช้ .pack()
...
# ======================================================================
# (อย่าลืม import CTkFont, partial, messagebox)
...
def create_pet_details_page(root, bg_images, pet_id, prev_page_func):
    ...
    font_price = CTkFont(family="Josefin Sans Bold", size=18, weight="bold")
    ...
    ctk.CTkLabel(root, text=f"Price: {pet_data['price']:,.0f} ฿", font=font_price, text_color="#d9534f", fg_color="white").place(x=x, y=y+4*gap)
    ...
    def add_cart():
        ...
        for item in CART:
            if item['id'] == pet_data['id']:
                messagebox.showinfo("Already in Cart", f"{pet_data['breed']} is already in your cart.")
                return

        # เพิ่มข้อมูลสัตว์ลงตะกร้า
        CART.append({
            "id": pet_data['id']
            # (เราเก็บแค่ ID ก็พอครับ เพราะหน้าตะกร้าจะไปดึงข้อมูลเต็มๆ มาเอง)
        })
    ...
    # ---- About Button (ผมลบออกไปตอนแปลงโค้ดครั้งที่แล้ว แต่เติมกลับให้ครับ) ----
...
# หน้า Cart
# ======================================================================
# (ฟังก์ชันนี้ควรอยู่ข้างนอก)
def remove_item_from_cart(item_index, root, bg_images, prev_page_func):
    """ลบไอเทมออกจาก CART ตาม index แล้วโหลดหน้านี้ใหม่"""
    try:
        CART.pop(item_index)
        create_cart_page(root, bg_images, prev_page_func) # โหลดหน้าตะกร้าซ้ำ
    except IndexError:
        print(f"Error: ไม่สามารถลบ index {item_index} ได้")

# (ฟังก์ชันนี้ควรอยู่ข้างนอก)
def create_cart_page(root, bg_images, prev_page_func):
    clear_page(root)
    root.title("MY CART") # <--- แก้ Title ให้ถูกต้อง

    # --- 1. สร้าง Fonts ---
    font_title = CTkFont(family="Josefin Sans Bold", size=32)
    font_header = CTkFont(family="Josefin Sans Bold", size=16)
    font_item = CTkFont(family="Josefin Sans", size=14)
    font_total = CTkFont(family="Josefin Sans Bold", size=18)
    font_checkout = CTkFont(family="Josefin Sans Bold", size=20)
    font_empty = CTkFont(family="Josefin Sans", size=22)
    ...
    # --- 4. สร้าง Header ของตาราง ---
    header_frame = ctk.CTkFrame(root, fg_color="#FFFFFF", corner_radius=10, height=60)
    header_frame.place(relx=0.5, y=140, relwidth=0.85, anchor="n")

    headers = ["Pet", "Breed", "Age", "Gender", "Color", "Price", "Remove"]
    col_widths = [0.1, 0.2, 0.1, 0.1, 0.15, 0.15, 0.1]
    x_pos = 0.02 

    for i, header in enumerate(headers):
        ctk.CTkLabel(header_frame, text=header, font=font_header, text_color="#333").place(
            relx=x_pos, rely=0.5, anchor="w"
        )
        x_pos += col_widths[i]
    ...
    for index, item_in_cart in enumerate(CART): 
        try:
            pet_id = item_in_cart['id'] # <-- ดึง 'id' จาก dict
            pet = get_pet_data_by_id(pet_id) 
            if not pet:
                print(f"คำเตือน: ไม่พบ Pet ID {pet_id} ในฐานข้อมูล")
                continue
            pet_type = pet.get('type', 'N/A')
            pet_breed = pet.get('breed', 'N/A')
            pet_age = pet.get('age', 'N/A')
            pet_gender = pet.get('gender', 'N/A')
            pet_color = pet.get('color', 'N/A')
            pet_price = pet.get('price', 0)
            
            total_price += pet_price

            row_frame = ctk.CTkFrame(root, fg_color="#FFFFFF", height=50)
            row_frame.place(relx=0.5, y=y_pos, relwidth=0.85, anchor="n")
            
            x_pos = 0.02 # Reset x_pos
            
            ctk.CTkLabel(row_frame, text=pet_type, font=font_item, text_color="#007bff").place(relx=x_pos, rely=0.5, anchor="w")
            x_pos += col_widths[0]
            ctk.CTkLabel(row_frame, text=pet_breed, font=font_item, text_color="#333").place(relx=x_pos, rely=0.5, anchor="w")
            x_pos += col_widths[1]
            ctk.CTkLabel(row_frame, text=str(pet_age), font=font_item, text_color="#333").place(relx=x_pos, rely=0.5, anchor="w")
            x_pos += col_widths[2]
            ctk.CTkLabel(row_frame, text=pet_gender, font=font_item, text_color="#333").place(relx=x_pos, rely=0.5, anchor="w")
            x_pos += col_widths[3]
            ctk.CTkLabel(row_frame, text=pet_color, font=font_item, text_color="#333").place(relx=x_pos, rely=0.5, anchor="w")
            x_pos += col_widths[4]
            ctk.CTkLabel(row_frame, text=f"฿{pet_price:,.0f}", font=font_item, text_color="#333").place(relx=x_pos, rely=0.5, anchor="w")
            x_pos += col_widths[5]
            
            remove_func = partial(remove_item_from_cart, index, root, bg_images, prev_page_func)
            ctk.CTkButton(
                row_frame, text="⛔", width=25, height=25, 
                fg_color="#ff9bb3", hover_color="#D32F2F",
                command=remove_func
            ).place(relx=x_pos, rely=0.5, anchor="w")

        except Exception as e:
            print(f"Error processing cart item: {e}")
            ctk.CTkLabel(root, text=f"Error displaying item {item_in_cart.get('id', 'N/A')}", font=font_item, text_color="red").place(x=200, y=y_pos)

        y_pos += 60 

    # --- 7. สรุปยอดรวม และปุ่มต่างๆ ---
    ctk.CTkLabel(root, text=f"Total : ฿{total_price:,.0f}", 
                 font=font_total, text_color="#005f99"
                 ).place(x=950, y=y_pos + 20, anchor="e")

    ctk.CTkButton(
        root, text="Confirm order",
        font=font_checkout,
        fg_color="#76c893",
        hover_color="#5fa27a",
        width=200, 
    ...
    ).place(x=950, y=y_pos + 60, anchor="e")
...
def create_marketplace_page(root, bg_images, user_role, selected_category="All"):
    ...
    scroll_area = ctk.CTkScrollableFrame(root, fg_color="transparent")
    # .place() ให้มันกินพื้นที่เกือบทั้งหมดที่เหลือ
    scroll_area.place(x=10, y=125, relwidth=0.98, relheight=0.8)
    ...
    #    เพื่อที่ปุ่ม Back ของหน้านั้น จะได้รู้ว่าต้องกลับมาหน้านี้
        ...
        # Frame การ์ด
        frame = ctk.CTkFrame(scroll_area, width=200, height=260, fg_color="#ffffff", corner_radius=12)
        #คำนวณแถว/คอลัมน์
        row = i // 6  # <-- 6 ตัวต่อแถว
        col = i % 6   # <-- 6 ตัวต่อแถว
        ...
        # Age + Price
        info_lbl = ctk.CTkLabel(frame, text=f"{age} | ฿{price:,.0f}",
